...
class EamFileWriter:
  ...
  # write header for eam file
    ...
    if len(self.elist)==1:
      ...
      raise Exception("Only two elements are supported as this time.")

    header_template = '{0:5d}{1:24.16e} {2:5d}{3:24.16e}{4:24.16f}\n'
    self.file.write(header_template.format(self.Nrho, self.drho, self.Nr, self.dr, self.globalcutoff))

  def write_singleeam(self):
  #Write Single atom section of  EAM/ALLOY style (funcfl) Potential for LAMMPS

    single_eam_template = '{0:5d}{1:15.5g}{2:15.5g} {3:s}\n'
    self.file.write(single_eam_template.format(self.elements.atnum,
                                               self.elements.mass,
                                               self.elements.alat,
