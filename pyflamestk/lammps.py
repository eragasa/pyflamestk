...
class Potential:
    def __init__(self,symbols):
        ...
        self._is_charge = None # determines whether this is a charged
                               # potential
    @property
    def symbols(self):
...
class BuckinghamPotential(Potential):
    ...
    def write_potential_file(self,fname_out,param_dict):
        assert type(fname_out),str
        assert type(param_dict),dict
        elements = self._symbols
        charge = {}
        for e in elements:
            charge[e] = param_dict['chrg_{}'.format(e)]
        str_pot = ''

        # set masses
        for i,e in enumerate(elements):
            str_pot += "mass {} {}\n".format(i+1,self._get_mass(e))
        str_pot += '\n'

        # set groups
        for i,e in enumerate(elements):
            str_pot += "group {} type {}\n".format(self._get_name(e),i+1)
        str_pot += '\n'

        # set charge for groups
        for i,e in enumerate(elements):
            str_pot += "set group {} charge {}\n".format(self._get_name(e),
                                                         charge[e])
        str_pot += '\n'
        ...
        for i, element_i in enumerate(elements):
            for j, element_j in enumerate(elements):
                if i <= j:
                    try:
                        p_rho = param_dict['{}{}_rho'.format(element_i,element_j)]
                        p_a   = param_dict['{}{}_A'.format(element_i,element_j)]
                        p_c   = param_dict['{}{}_C'.format(element_i,element_j)]
                    except KeyError:
                        # if i,j potential not found, it must be j,i
                        p_rho = param_dict['{}{}_rho'.format(element_j,element_i)]
                        p_a   = param_dict['{}{}_A'.format(element_j,element_i)]
                        p_c   = param_dict['{}{}_C'.format(element_j,element_i)]
