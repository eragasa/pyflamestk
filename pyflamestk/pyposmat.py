...
class PyPosmatEngine:
  """
  Attributes:
      supported_qoi: an array of quantities of interest which can be calculated
      qoi: a dictionary
      lammps_sims: a dictionary:
      dir_lammps_sim_db: the directory of lammps simulation templates
  ...
  def __init__(self, 
    ...
    # list of supported quantities of interest
  ...
  def evaluate_parameter_set(self, potential_parameters):
      ...
      for simulation in self.lammps_sims:
          ...
          if sim_code == 'sp':
              ...
              self.lammps_sims[simulation]['sim_type']  = 'elastic'
          elif sim_code == 'min':
              self.lammps_sims[simulation]['sim_type']  = 'min'
          else:
  ...
  def run_all_lammps_simulations(self):
    ...
    if(sim_has_error):
      msg_err = "simulation has failed"
      raise PyPosmatError(msg_err)
  ...
  def determine_lammps_simulations(self):
      ...
      # supported by single point calculations
      qoi_req_sp = ['sp_ecoh','sp_pressure']

      # supported by the elastic simulations
      qoi_req_elas = ['elas_c11','elas_c12','elas_c44',
                      'elas_bulk','elas_shear']    
      
      # supported by geometric relaxation
      qoi_req_geo_relax = ['min_latt_xx','min_latt_yy',
                           'min_latt_zz','min_ecoh']

      for qoi in self.qoi_list:
          lammps_sim_name = "{}.{}"
          structure = self.qoi[qoi]['structure'][0]

          if self.qoi[qoi]['variable'] in qoi_req_sp:
              lammps_sim_template = "sp"
          elif self.qoi[qoi]['variable'] in qoi_req_elas:
              lammps_sim_template = "elas"
          elif self.qoi[qoi]['variable'] in qoi_req_geo_relax:
              lammps_sim_template = "min"
          else:
  ...
  def write_lammps_buckingham_potential_file(self, potential_params, fname_out):
      ...
      for element in elements:
          charge[element] = potential_params['chrg_{}'.format(element)]
      str_pot = ""
          ...
          for j, element_j in enumerate(elements):
              if i <= j:
                  try:
                      ...
                      # if i,j potential not found, it must be j,i
                      p_rho = potential_params['p_{}{}_rho'.format(element_j,element_i)]
                      p_a   = potential_params['p_{}{}_a'.format(element_j,element_i)]
                      p_c   = potential_params['p_{}{}_c'.format(element_j,element_i)]
                      
                  str_pot += "pair_coeff {} {} {} {} {} {}\n".format(i+1,j+1,p_a,p_rho,p_c,'${R_cut}')
      str_pot += "\n"
  ...
  def create_lammps_simulation(self,
      ...
      if not(fname_potential == None):
        ...
        if dir_name.endswith('.elas'):
           # the potential file for elastic simulation requires some other restart variables
           f = open(dst_fname,'a')
        ...
        with open(new_fname,'w') as new_file:
          with open(old_fname,'r') as old_file:
            for line in old_file:
              if line.startswith('atom_style'):
                new_file.write("atom_style charge\n")
              else:
                new_file.write(line)
  ...
  def calculate_qoi(self):
    ...
    self.abs_error = {}
    self.abs_error_normalized = {}
    self.square_error = {}
    self.square_error_normalized = {}

    for key in self.qoi_list:
      self.abs_error[key] = abs(self.qoi[key]['est'] - self.qoi[key]['target'])
      self.abs_error_normalized[key] = self.abs_error[key]/self.qoi[key]['norm']
      self.square_error[key] = self.abs_error[key]**2
      self.square_error_normalized[key] = self.square_error[key]/(self.qoi[key]['norm']**2.0)
  ...
  def get_properties_from_simulations(self):
    ...
    for qoi_key in self.qoi_list:
      ...
      if variable_name == 'min_latt_xx':
        simulation      = "{}.{}".format(structures[0],'min')
        ...
        for structure in structures:
          ...
          self.properties[property_name] = pyflamestk.lammps.getCohesiveEnergy(os.path.join(simulation,'out.dat'))
      elif variable_name in ['elas_c11', 'elas_c12', 'elas_c44', 'elas_bulk', 'elas_shear']:
        simulation      = "{}.{}".format(structures[0],'elas')
        ...
        msg_err = "pyposmat qoi not supported: {}"
        msg_err = msg_err.format(variable_name)
        raise ValueError(msg_err)
  ...
  def get_qoi_estimates(self):
    for qoi_key in self.qoi_list:
      ...
      elif variable_name == 'elas_c11':
          self.qoi[qoi_key]['est'] = self.properties["{}.{}".format(structures[0],'c11')]
      elif variable_name == 'elas_c12':
          self.qoi[qoi_key]['est'] = self.properties["{}.{}".format(structures[0],'c12')]
      elif variable_name == 'elas_c44':
          self.qoi[qoi_key]['est'] = self.properties["{}.{}".format(structures[0],'c44')]
      elif variable_name in ['elas_bulk']:
      ...
      elif variable_name in ['elas_shear']:
          c11 = self.properties["{}.c11".format(structures[0])]
          c12 = self.properties["{}.c12".format(structures[0])]
...
class FileParameterSampler(PyPosmatEngine):
  ...
  def get_new_qoi_set(self):
    # calculate qois from simulation results
    new_qoi_set = []
    i = 0
    for qoi_key in self.qoi_list:
        new_qoi_set.append(self.qoi[qoi_key]['est'])
        new_qoi_set.append(self.abs_error[qoi_key])
        new_qoi_set.append(self.abs_error_normalized[qoi_key])
